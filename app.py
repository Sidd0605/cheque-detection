...
# Function to process the uploaded image in-memory
def process_image(image_file):
    ...
    if ordered_cropped_images:
        ...
        # Save the final combined image to an in-memory file
        img_io = BytesIO()
...
def predict():
    ...
    if file:
        ...
        result_image_io = process_image(file)

        if result_image_io:
            # Send the result image as a file response without saving it to disk
            return send_file(result_image_io, mimetype='image/jpeg')
        else:
            return render_template('index.html', error="No objects detected")
